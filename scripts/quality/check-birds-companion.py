...
def main():
 ...
 with tempfile.TemporaryDirectory(prefix='cobalt-birds-check-',dir='/tmp') as temp:
  ...
  snapshot.write_text(json.dumps({'format':'cobalt-birds-v1','generated_at':2})+'\n');pending=shelf/'.current.png.writing';pending.write_bytes(b'other');run('push',snapshot,image,'--sim',ok=False);assert pending.read_bytes()==b'other' and (shelf/'current.json').read_bytes()==before[0]
 a.output.mkdir(parents=True,exist_ok=True);(a.output/'transcript.json').write_text(json.dumps(transcript,indent=2)+'\n');(a.output/'result.json').write_text(json.dumps({'status':'passed','physical_hardware':False,'checks':['valid v1 snapshot published','invalid JSON preserves prior snapshot','occupied staging path preserves prior unit']},indent=2)+'\n')
