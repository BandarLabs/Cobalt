...
impl Lifecycle {
    const fn name(self) -> &'static str {
        match self {
            ...
            Self::NickelPauseRequested => "nickel_pause_requested",
            Self::NickelPaused => "nickel_paused",
            ...
            Self::NickelResumeRequested => "nickel_resume_requested",
            Self::NickelResumed => "nickel_resumed",
    ...
    fn parse(value: &str) -> Option<Self> {
        Some(match value.trim() {
            ...
            "nickel_pause_requested" => Self::NickelPauseRequested,
            "nickel_paused" => Self::NickelPaused,
            ...
            "nickel_resume_requested" => Self::NickelResumeRequested,
            "nickel_resumed" => Self::NickelResumed,
...
impl Sampler {
    ...
    fn sample_core(&mut self) -> Snapshot {
        ...
        let resolver_identity = resolver_content_identity(&resolver_text, &self.privacy);
    ...
    fn processes(&mut self) -> Vec<ProcessRecord> {
        ...
        for entry in entries.flatten().take(512) {
            ...
            let Some((parent_pid, starttime, state)) = parse_process_stat(&stat) else {
                continue;
            };
            ...
            records.push(ProcessRecord {
                ...
                identity,
                ppid: parent_pid,
                state: process_state_category(state),
...
/// Verifies the clean single-generation ownership required before the
/// attended same-process Nickel pause candidate may run.
///
/// This performs only the same read-only process, socket, route, D-Bus,
/// association, and address observations used by the tracer.
///
/// # Errors
///
/// Returns privacy-safe invariant names when ownership is absent, duplicated,
/// uninterruptible, or not presently healthy.
pub fn verify_pause_resume_preflight() -> Result<(), Vec<&'static str>> {
    let privacy = PrivacyKey::generate().map_err(|_| vec!["privacy key unavailable"])?;
    let mut sampler = Sampler::device(privacy.clone());
    sampler.slow.wpa_state = read_wpa_state();
    sampler.slow.ipv4 = read_ipv4_category();
    sampler.slow.dbus_owner = read_dbus_owner(&privacy);
    let snapshot = sampler.sample_core();
    let problems = pause_resume_preflight_problems(&snapshot);
...
fn pause_resume_preflight_problems(snapshot: &Snapshot) -> Vec<&'static str> {
    let mut problems = Vec::new();
    for (role, count, label) in [
        ("nickel", snapshot.nickel_count, "Nickel count is not one"),
        (
            "supplicant",
            snapshot.supplicant_count,
            "supplicant count is not one",
        ),
        ("dhcp", snapshot.dhcp_count, "DHCP count is not one"),
        (
            "dhcpcd_dbus",
            snapshot.dbus_process_count,
            "dhcpcd D-Bus process count is not one",
        ),
        (
            "wmt_launcher",
            snapshot.wmt_launcher_count,
            "WMT launcher count is not one",
        ),
    ] {
        if count != 1 {
            problems.push(label);
        ...
        if snapshot.processes.iter().any(|process| {
            process.role == role
                && matches!(
                    process.state,
                    "stopped" | "uninterruptible" | "zombie" | "other"
                )
        }) {
            problems.push(match role {
                "nickel" => "Nickel process state is unsafe",
                "supplicant" => "supplicant process state is unsafe",
                "dhcp" => "DHCP process state is unsafe",
                "dhcpcd_dbus" => "dhcpcd D-Bus process state is unsafe",
                _ => "WMT launcher process state is unsafe",
            });
        }
    }
    if !snapshot.wlan_present {
        problems.push("wlan0 is missing");
    }
    if snapshot.wpa_socket == "absent" {
        problems.push("wpa control socket is missing");
    }
    if snapshot.dbus_owner == "absent" || snapshot.dbus_owner == "tool_missing" {
        problems.push("dhcpcd D-Bus owner is unavailable");
    }
    if snapshot.wpa_state != "completed" {
        problems.push("wpa association is not completed");
    }
    if matches!(
        snapshot.ipv4.as_str(),
        "none" | "tool_missing" | "unavailable"
    ) {
        problems.push("wlan0 IPv4 address is unavailable");
    }
    if !snapshot.route.default_present || !snapshot.route.up || !snapshot.route.gateway {
        problems.push("healthy wlan0 default route is unavailable");
    }
    if snapshot.route.default_count != 1 {
        problems.push("wlan0 default route count is not one");
    }
    if snapshot.resolver_count == 0 || snapshot.resolver == "none" {
        problems.push("resolver is unavailable");
    }
    problems
}
    ...
    fn open_with_limit(path: &Path, active_probes: bool, limit: u64) -> io::Result<Self> {
        ...
        writer.line(
            &ObjectBuilder::new()
                ...
                .set("mono_ms", 0_u32)
                .set("variant", "n365_same_process_pause_resume")
                .set("base_commit", "ea08a08fd7b7ccb292e7e2346f0d3ad98713d9f9")
                .set(
                    "kind",
                    if existed {
                        "tracer_restart"
...
fn lifecycle_requests_reference(event: Lifecycle) -> bool {
    matches!(
        event,
        Lifecycle::Current94GateAccepted | Lifecycle::NickelResumed
    )
}
...
fn snapshot_value(
    ...
    ObjectBuilder::new()
        ...
        .set("resolver_identity", snapshot.resolver_identity.clone())
...
fn parse_process_stat(stat: &str) -> Option<(i32, u64, char)> {
    ...
    Some((ppid, starttime, state))
}

fn process_state_category(state: char) -> &'static str {
    match state {
        'T' | 't' => "stopped",
        'D' => "uninterruptible",
        'R' => "running",
...
fn resolver_content_identity(text: &str, privacy: &PrivacyKey) -> String {
    let relevant = text
        .lines()
        .map(str::trim)
        .filter(|line| {
    ...
    if relevant.is_empty() {
        ...
        privacy.identity(b"resolver-content", relevant.as_bytes())
    }
}
...
struct SummarySnapshot {
    ...
    resolver_search: bool,
    resolver_identity: String,
    dhcp_files: Vec<SummaryDhcpFile>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
struct SummaryDhcpFile {
    path_identity: String,
    kind: String,
    inode: u64,
    size: u64,
...
pub struct TraceSummary {
    ...
    pub anchor_snapshot_observed: bool,
    pub ended: bool,
...
impl TraceSummary {
    ...
    pub fn render(&self) -> String {
        ...
        let anchor_label = self.anchor_label.as_deref().unwrap_or("handoff anchor");
        lines.push(
            match (
                &self.first_divergence,
                self.gate_mono_ms,
                self.anchor_snapshot_observed,
            ) {
                (_, Some(anchor), false) => format!(
                "handoff not verified after {anchor_label} at {anchor}ms: no post-anchor snapshot"
            ),
                (Some(divergence), Some(gate), true) => {
                    format!("first divergence after {anchor_label} at {gate}ms: {divergence}")
                }
                (None, Some(gate), true) => {
                    format!("no later divergence recorded after {anchor_label} at {gate}ms")
                }
                _ => "no handoff anchor checkpoint was recorded".to_owned(),
            },
        );
        ...
        match value.get("kind").and_then(Value::as_str) {
            ...
            Some("lifecycle")
                if matches!(
                    ...
                    Some("current_94_gate_accepted" | "nickel_resumed")
                ) =>
            {
                summary.anchor_label = value.get("event").and_then(Value::as_str).map(|event| {
                    if event == "nickel_resumed" {
                        "same-process Nickel resume"
                    } else {
                        "#94 gate"
                    }
                    .to_owned()
            ...
            Some("snapshot") => {
                ...
                if gate_pending {
                    summary.anchor_snapshot_observed = true;
...
fn summary_snapshot(value: &Value) -> Option<SummarySnapshot> {
    ...
    Some(SummarySnapshot {
        ...
        resolver_count: number("resolver_count")?,
        resolver_search: boolean("resolver_search")?,
        resolver_identity: text("resolver_identity")?,
        dhcp_files: summary_dhcp_files(value.get("dhcp_files")?)?,
    })
}

fn summary_dhcp_files(value: &Value) -> Option<Vec<SummaryDhcpFile>> {
    value
        .as_array()?
        .iter()
        .map(|file| {
            ...
            let signed = |name: &str| file.get(name).and_then(Value::as_i64);
            let text = |name: &str| file.get(name).and_then(Value::as_str).map(str::to_owned);
            let content_identity = match file.get("content_identity")? {
                Value::Null => None,
                value => Some(value.as_str()?.to_owned()),
            };
            Some(SummaryDhcpFile {
                path_identity: text("path_identity")?,
                kind: text("kind")?,
                inode: number("inode")?,
                size: number("size")?,
                mtime: signed("mtime")?,
...
fn immediate_comparative_divergence(
    ...
    if before.resolver != after.resolver
        || before.resolver_count != after.resolver_count
        || before.resolver_search != after.resolver_search
        || before.resolver_identity != after.resolver_identity
    {
        return Some("resolver state changed across gate acceptance");
    }
    if before.dhcp_files != after.dhcp_files {
        return Some("DHCP artifact identity changed across gate acceptance");
    }
    for (prefix, description) in [
        ...
        (
            'W',
            "WMT launcher process generation changed across gate acceptance",
        ),
    ] {
...
fn divergence(reference: &SummarySnapshot, current: &SummarySnapshot) -> Option<&'static str> {
    ...
    for (prefix, description) in [
        ('N', "Nickel process generation changed after handoff"),
        ('S', "supplicant process generation changed after handoff"),
        ('D', "DHCP process generation changed after handoff"),
        ('B', "dhcpcd D-Bus process generation changed after handoff"),
        ('W', "WMT launcher process generation changed after handoff"),
    ] {
        if generation_part(&reference.tuple, prefix) != generation_part(&current.tuple, prefix) {
            return Some(description);
        }
    }
    ...
    if reference.resolver != current.resolver
        || reference.resolver_count != current.resolver_count
        || reference.resolver_search != current.resolver_search
        || reference.resolver_identity != current.resolver_identity
    {
        return Some("resolver state changed after handoff");
    }
    if reference.dhcp_files != current.dhcp_files {
        return Some("DHCP artifact identity changed after handoff");
    }
    None
}
...
mod tests {
    ...
    fn healthy(tuple: &str) -> Snapshot {
        Snapshot {
            ...
            resolver_identity: "hmac:resolver-one".to_owned(),
            dhcp_files: Vec::new(),
            probes: ProbeState::disabled(),
    ...
    fn ownership_divergences_are_classified() {
        ...
        let cases = [
            ...
            ("resolver state", {
                let mut state = baseline.clone();
                state.resolver_identity = "hmac:resolver-two".to_owned();
                state
            }),
            ("DHCP artifact identity", {
                let mut state = baseline.clone();
                state.dhcp_files.push(FileRecord {
                    path_identity: "hmac:path".to_owned(),
                    kind: "file",
                    inode: 10,
                    size: 20,
                    mtime: 30,
                    mtime_nsec: 40,
                    content_identity: Some("hmac:content".to_owned()),
                });
                state
            }),
            ("deleted after", {
    ...
    #[test]
    fn same_process_resume_is_a_distinct_handoff_anchor() {
        let snapshot = healthy("N1;S1;D1;B1;B=owner:one;C=inode:1");
        let trace = String::from_utf8(trace(&[snapshot.clone(), snapshot]))
            .expect("trace text")
            .replace("current_94_gate_accepted", "nickel_resumed");
        let summary = summarize(trace.as_bytes());
        assert_eq!(
            summary.anchor_label.as_deref(),
            Some("same-process Nickel resume")
        );
        assert!(summary.first_divergence.is_none());
        assert!(lifecycle_requests_reference(Lifecycle::NickelResumed));
        assert!(lifecycle_requests_reference(
            Lifecycle::Current94GateAccepted
        ));
        assert!(!lifecycle_requests_reference(Lifecycle::NickelPaused));
    }

    #[test]
    fn an_anchor_without_a_post_anchor_snapshot_is_not_reported_clean() {
        let mut line = ObjectBuilder::new()
            .set("version", TRACE_VERSION)
            .set("mono_ms", 10_u32)
            .set("kind", "lifecycle")
            .set("event", "nickel_resumed")
            .build()
            .to_json();
        line.push('\n');
        let summary = summarize(line.as_bytes());
        assert!(!summary.anchor_snapshot_observed);
        assert!(summary.render().contains("handoff not verified"));
        assert!(!summary.render().contains("no later divergence recorded"));
    }
    ...
    #[test]
    fn single_process_generation_replacements_after_anchor_are_divergences() {
        let baseline = healthy("N1;S1;D1;B1;W1;B=owner:one;C=inode:1");
        let cases = [
            (
                "Nickel process generation",
                "N2;S1;D1;B1;W1;B=owner:one;C=inode:1",
            ),
            (
                "supplicant process generation",
                "N1;S2;D2;B1;W1;B=owner:one;C=inode:1",
            ),
            (
                "DHCP process generation",
                "N1;S1;D2;B1;W1;B=owner:one;C=inode:1",
            ),
            (
                "D-Bus process generation",
                "N1;S1;D1;B2;W1;B=owner:one;C=inode:1",
            ),
            (
                "WMT launcher process generation",
                "N1;S1;D1;B1;W2;B=owner:one;C=inode:1",
            ),
        ];
        for (expected, tuple) in cases {
            let mut changed = baseline.clone();
            changed.tuple = tuple.to_owned();
            let summary = summarize(&trace(&[baseline.clone(), changed.clone(), changed]));
            assert!(
                summary
                    .first_divergence
                    .as_deref()
                    .is_some_and(|value| value.contains(expected)),
                "{expected} was only listed as a transition: {summary:?}"
            );
        }
    }
    ...
    fn process_stat_distinguishes_starttime() {
        ...
        assert_eq!(parse_process_stat(stat), Some((1, 98765, 'S')));
    }
    ...
    #[test]
    fn multiple_wmt_launchers_are_visible_at_handoff() {
        let mut snapshot = healthy("N1;S1;D1;B1;W1+2;B=owner:one;C=inode:1");
        snapshot.wmt_launcher_count = 2;
        snapshot.processes.push(process("wmt_launcher", 51, 501, 2));
        snapshot.processes.last_mut().expect("WMT process").state = "uninterruptible";
        let problems = pause_resume_preflight_problems(&snapshot);
        assert!(problems.contains(&"WMT launcher count is not one"));
        assert!(problems.contains(&"WMT launcher process state is unsafe"));
    ...
    #[test]
    fn clean_single_generation_state_passes_pause_preflight() {
        let healthy = healthy("N1;S1;D1;B1;W1;B=owner:one;C=inode:1");
        assert!(pause_resume_preflight_problems(&healthy).is_empty());
        let mut ambiguous = healthy.clone();
        ambiguous.route.default_count = 2;
        ambiguous.resolver = "none".to_owned();
        ambiguous.resolver_count = 0;
        let problems = pause_resume_preflight_problems(&ambiguous);
        assert!(problems.contains(&"wlan0 default route count is not one"));
        assert!(problems.contains(&"resolver is unavailable"));
    }
    ...
    fn collection_omits_representative_owner_data() {
        ...
        snapshot.resolver_identity = first_key.identity(b"resolver-content", raw.as_bytes());
