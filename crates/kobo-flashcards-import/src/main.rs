...
fn run(arguments: &[String]) -> Result<(), String> {
    ...
    if matches!(arguments, [operation, _] if operation == "verify") {
        let bundle = &arguments[1];
        let report =
            verify_bundle(PathBuf::from(bundle).as_path()).map_err(|error| error.to_string())?;
        println!(
            "verified: {} notes, {} active cards, {} digest-verified media files; {} image-bearing notes resolve to bundled bytes; {} sound-bearing notes are retained",
            report.notes,
            report.active_cards,
            report.media_files,
    ...
    if matches!(arguments, [operation, flag, root, output] if operation == "export-review-log" && flag == "--kobo-root")
    {
        let report = export_local_review_log(
            PathBuf::from(&arguments[2]).as_path(),
            PathBuf::from(&arguments[3]).as_path(),
        )
        .map_err(|error| error.to_string())?;
        println!(
    ...
    .map_err(|error| error.to_string())?;
    println!(
        "imported {}: {} notes, {} active cards, {} decks, {} media files ({} bytes), {} image-bearing notes resolved, {} sound-bearing notes retained",
        report.package_kind,
        report.notes,
        report.active_cards,
        report.decks,
