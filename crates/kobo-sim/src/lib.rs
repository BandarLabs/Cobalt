            ...
            for y in top..top.saturating_add(height) {
                ...
                for x in left..left.saturating_add(width) {
                    ...
                    *visible = u8::try_from((u16::from(target) * 15 + u16::from(*visible)) / 16)
                    ...
                    for channel in 0..3 {
                        let channel_target = if matches!(update.waveform, PanelWaveform::Du) {
                            target
                        } else {
                            self.ideal_rgb
                        ...
                        *visible = u8::try_from(
                            (u16::from(channel_target) * 15 + u16::from(*visible)) / 16,
                        )
                        .unwrap_or(channel_target);
                    }
                }
            }
        }
