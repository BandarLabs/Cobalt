...
fn wifi_trace_command(arguments: &[String]) -> Result<(), String> {
    const USAGE: &str = "usage: kobo wifi-trace summarize PATH | \
                         kobo wifi-trace retrieve --device HOST --trace TRACE.jsonl --out PATH";
    match arguments {
        ...
        [action, device, host, trace_flag, trace_name, out, path]
            if action == "retrieve"
                && is_device_flag(device)
                && trace_flag == "--trace"
                && out == "--out" =>
        {
            if !valid_device_host(host) {
            ...
            if !valid_wifi_trace_name(trace_name) {
                return Err("trace name is not a Cobalt Wi-Fi handoff trace".to_owned());
            }
            let script = format!(
                "set -e\n\
                 dir={directory}\n\
                 trace=\"$dir/{trace_name}\"\n\
                 if [ ! -f \"$trace\" ]; then\n\
                   echo 'the requested Wi-Fi handoff trace is not on this device' >&2\n\
                   exit 3\n\
                 fi\n\
                 cat \"$trace\"\n",
...
fn valid_wifi_trace_name(name: &str) -> bool {
    name.strip_prefix("wifi-handoff-v1-")
    ...
    println!(
        "Kobo application SDK\n\n\
         ...
         Commands:\n\
           ...
           wifi-trace retrieve --device IP --trace NAME --out PATH  Retrieve one exact trace\n\
...
mod tests {
    ...
    use super::{
        ...
        parse_touch_probe, unreachable_device, valid_device_host, valid_slug,
        valid_wifi_trace_name, verify_arm_elf, wait_for_remote_child, workspace_doctor_binary,
        DevSessionGuard, RemoteArtifact, SimulationGuard, ALIASES, DEFAULT_TRACE_LINES,
        DEPLOY_TIMEOUT, DEVICE_PACKAGES, TOUCH_PROBE_DEFAULT_SECONDS, TOUCH_PROBE_MAXIMUM_SECONDS,
    };
    #[cfg(feature = "device-write")]
    use super::{
    ...
    #[test]
    fn wifi_trace_retrieval_names_one_exact_session() {
        assert!(valid_wifi_trace_name("wifi-handoff-v1-46880-1095.jsonl"));
        for rejected in [
            "wifi-handoff-v1-.jsonl",
            "wifi-handoff-v1-latest.jsonl",
            "wifi-handoff-v1-1.jsonl;reboot",
            "../wifi-handoff-v1-1-2.jsonl",
            "wifi-handoff-baseline-v1-1-2.jsonl",
        ] {
            assert!(!valid_wifi_trace_name(rejected), "{rejected}");
        }
    }
