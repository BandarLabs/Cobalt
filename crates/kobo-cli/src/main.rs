    ...
    match arguments {
            ...
            if action == "retrieve" && is_device_flag(device) && out == "--out" =>
        {
            ...
            let script = format!(
                "set -e\n\
                 dir={directory}\n\
                 latest=$(ls -1t \"$dir\"/wifi-handoff-baseline-v1-*.jsonl 2>/dev/null | head -n 1)\n\
                 if [ -z \"$latest\" ]; then\n\
                   echo 'no baseline Wi-Fi handoff trace on this device' >&2\n\
                   exit 3\n\
                 fi\n\
                 cat \"$latest\"\n",
            ...
            if !output.status.success() {
                return Err(match output.status.code() {
                    Some(3) => "no baseline Wi-Fi handoff trace to retrieve".to_owned(),
                    _ => unreachable_device(format!(
                        "retrieving the baseline Wi-Fi handoff trace from {host} failed"
                    )),
                });
            }
    ...
    println!(
        "Kobo application SDK\n\n\
         ...
         Commands:\n\
           ...
           wifi-trace summarize PATH  Report baseline handoff generations and first divergence\n\
           wifi-trace retrieve --device IP --out PATH  Retrieve and summarize the latest baseline trace\n\
