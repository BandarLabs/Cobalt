...
pub fn render_oriented_with_turn(
    ...
    let mut logical = if surface.rgb.is_some() {
        Surface::new_color(surface.height, surface.width)
    } else {
        Surface::new(surface.height, surface.width)
    };
    render_all(
        screen,
...
fn rotate_landscape(logical: &Surface, physical: &mut Surface, turn: LandscapeTurn) {
    for y in 0..logical.height {
        for x in 0..logical.width {
            ...
            let source = y * logical.width + x;
            physical.pixels[destination] = logical.pixels[source];
            if let (Some(logical_rgb), Some(physical_rgb)) = (&logical.rgb, &mut physical.rgb) {
                let src = source * 3;
                let dst = destination * 3;
                if let (Some(src_rgb), Some(dst_rgb)) = (
                    logical_rgb.get(src..src + 3),
                    physical_rgb.get_mut(dst..dst + 3),
                ) {
                    dst_rgb.copy_from_slice(src_rgb);
                }
            }
        }
    }
