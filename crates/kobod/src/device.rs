...
impl Painter {
    ...
    fn apply_region(
        ...
        let frame = if matches!(update.waveform, PanelWaveform::Color) {
            let rgb = surface
                .rgb
                .as_ref()
                .ok_or_else(|| "the color transition has no RGB surface".to_owned())?;
            let x = usize::try_from(update.region.x)
                .map_err(|_| "the transition region is not inside the surface".to_owned())?;
            let y = usize::try_from(update.region.y)
                .map_err(|_| "the transition region is not inside the surface".to_owned())?;
            let width = usize::try_from(update.region.width)
                .map_err(|_| "the transition region is not inside the surface".to_owned())?;
            let height = usize::try_from(update.region.height)
                .map_err(|_| "the transition region is not inside the surface".to_owned())?;
        ...
        let plan = RefreshPlan::new(
            region,
            intent,
            matches!(update.waveform, PanelWaveform::Gc16 | PanelWaveform::Color),
            whole_screen.width,
            whole_screen.height,
        )
