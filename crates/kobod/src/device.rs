...
fn logo_bounds(mark: kobo_ui::Rect) -> kobo_ui::Rect {
    let width = mark.height.saturating_mul(LOGO_WIDTH) / LOGO_HEIGHT;
    kobo_ui::Rect {
        x: mark.x.saturating_add((mark.width - width) / 2),
        y: mark.y,
        width,
        height: mark.height,
    }
...
fn fill_cobalt_blue(surface: &mut Surface, rect: kobo_ui::Rect, colour_panel: bool) {
    if colour_panel {
        ...
        surface.fill_rect(rect, kobo_ui::luma(COBALT_BLUE));
    }
}
...
fn launch_surface(screen: &Screen, whole_screen: Rect, colour_panel: bool) -> Surface {
    ...
    if let Some(mark) = screen
        .layout_with(&metrics, &chrome)
        .nodes
        .iter()
        .find(|node| matches!(node.kind, LayoutKind::SplashGlyph(_)))
    {
        draw_cobalt_logo(&mut surface, logo_bounds(mark.rect), colour_panel);
    }
    surface
}
...
fn show_launch_screen(display: &DisplaySession, whole_screen: Rect) -> Result<(), String> {
    ...
    let surface = launch_surface(&screen, whole_screen, display.colour().is_some());
...
mod tests {
    ...
    fn launch_logo_keeps_its_blue_and_a_legible_greyscale_plane() {
        ...
        assert_eq!(
            ...
            kobo_ui::luma(super::COBALT_BLUE)
        );
