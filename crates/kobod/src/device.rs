...
impl Painter {
    ...
    fn apply_region(
        ...
        // A colour region is written in colour only where the panel can show
        // it. Everywhere else it is the quality update it would have been
        // before colour existed, drawn from the grey plane, which is what the
        // session would downgrade it to anyway. The decision is per region
        // rather than per frame: one transition can carry a colour picture and
        ...
        let plan = RefreshPlan::new(
            region,
            intent,
            // A colour region asks the controller for a full update for the
            // same reason a Gc16 one does: both are quality refreshes whose
            // point is a settled picture rather than a fast one.
