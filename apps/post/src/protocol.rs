...
pub fn letters(bytes: &[u8]) -> Vec<(String, String, String)> {
    ...
    root.as_array().map_or_else(Vec::new, |items| {
        items
            .iter()
            .filter_map(|letter| {
                Some((
                    letter.get("id")?.as_str()?.to_owned(),
                    letter.get("title")?.as_str()?.to_owned(),
                    letter.get("body")?.as_str()?.to_owned(),
                ))
            })
            .collect()
    })
...
mod tests {
    ...
    #[test]
    fn reads_completed_letters() {
        assert_eq!(
            letters(br#"[{"id":"dawn","title":"Morning note","body":"Tea first."}]"#)[0].1,
            "Morning note"
        );
    }
}
