...
pub enum Cadence { Manual, Hourly, FourHourly, Daily }
impl Cadence {
    pub const fn label(self) -> &'static str { match self { Self::Manual => "Manual only", Self::Hourly => "Hourly", Self::FourHourly => "Every 4 hours", Self::Daily => "Daily" } }
    pub const fn radio_minutes(self) -> u8 { match self { Self::Manual => 0, Self::Hourly => 120, Self::FourHourly => 30, Self::Daily => 5 } }
    pub const fn next(self) -> Self { match self { Self::Manual => Self::Hourly, Self::Hourly => Self::FourHourly, Self::FourHourly => Self::Daily, Self::Daily => Self::Manual } }
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct Config { pub device_id: String, pub cadence: Cadence, pub enabled: bool }
impl Default for Config { fn default() -> Self { Self { device_id: "KOBOSYNC-DEVICE-ID".into(), cadence: Cadence::Manual, enabled: false } } }
...
mod tests {
    use super::*;
    #[test] fn generated_config_binds_the_rest_api_to_loopback() {
    ...
    #[test] fn disabled_sync_never_opens_a_window() {
