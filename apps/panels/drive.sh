...
import struct, zlib
raw=Path("target/kobo-sim-last.raw").read_bytes(); w,h=1072,1448
assert len(raw)==w*h
...
out=Path("apps/panels/screenshots/library.png"); out.parent.mkdir(parents=True,exist_ok=True); out.write_bytes(png)
PY
