...
<td valign="top"><a href="examples/magnet/README.md"><img width="230" vspace="16" hspace="4" src="examples/magnet/screenshots/counting.png" alt="A screen counting how many times the hall sensor has changed as a magnet passes the bezel"></a><br><b><a href="examples/magnet/README.md">Magnet Sensor</a></b><br>The hall sensor behind the bezel, and where to find it.</td>
<td valign="top"></td>
</tr>
</table>
...
coming back is a repaint rather than a restart. That is what the recording at
the top is arranged around: the audiobook is started in its first minute and
left, everything else happens while it is being researched, written and
narrated, and the last thing on screen is the finished book on the shelf.
...
What a profile has to supply, how to measure it, and what refuses to work
until it is right are in **[docs/PORTING.md](docs/PORTING.md)**. If you have a
Libra, a Sage, a Clara 2E or anything else and you are willing to test on it,
open an issue first so the profile shape can be agreed before you write it.
...
Why Cobalt itself is not a `KoboRoot.tgz` is explained in
[docs/DEVICES.md](docs/DEVICES.md#why-cobalt-itself-is-not-a-koboroottgz).
...
report is in [docs/PORTING.md](docs/PORTING.md#how-to-get-the-numbers), and
...
keeping a reader awake while you work — is in
