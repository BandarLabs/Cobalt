...
2. **Submit/wait:** Use the device's existing timing smoke output. Retain requested intent, submitted/translated waveform, marker, submission and completion wait durations and restoration result. For the runtime, set `KOBO_FRAME_TIMING=1` in its environment before launch to capture `cobalt.refresh-observation` JSON on stderr. Correlate by session ID and marker; `submitted` is not `completed`, and either failure phase is a failed sample. The 128-record in-memory ring reports dropped records and is not a complete-run substitute for the captured log. Report sample count, median, p95 and maximum separately for GC16 and DU. Do not replace these with browser timestamps.
