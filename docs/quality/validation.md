...
- Actual simulator journeys pass for Backgammon, Fieldbook, Frame, Grimoire and Nonograms. Grimoire's revised route explicitly turns the class-choice page before selecting Wizard, then selects Abjuration and opens the filtered results. Captures are retained under `evidence/layout-integration` after the committed-source verification.
